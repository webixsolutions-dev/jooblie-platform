...
**R3.1 Branches & review:** `main` protected: PR required, green CI required, no direct push, aur no force push. **NORMAL (non-risky) changes ka standard solo path = documented self-review:** author full staged diff read karta hai, CI green confirm karta hai, PR mein applicable R5 checklist fill karta hai, phir merge karta hai. **R4-flagged RISKY changes:** merge se pehle second reviewer MANDATORY hai. Current solo setup mein ye reviewer org collaborator nahi, design/review channel ke through external architect hai. Risky change usi pass mein kabhi self-merge nahi hota jismein wo likha gaya: diff par STOP → external review → required fixes/verification → merge. Work branches: `hasham/<topic>`, `babar/<topic>`, `agent/<topic>` pattern. Long-lived personal branches nahi — branch = ek PR ka lifespan.
